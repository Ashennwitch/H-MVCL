...
\begin{table}[htbp]
\caption{Top-10 Global Feature Importance (Information Gain)}
\label{tab:rank}
\centering
\scriptsize % Reduces font size to save space
\begin{tabular}{l l c}
\hline
\textbf{Rank} & \textbf{Feature Name} & \textbf{Component View} \\
\hline
1  & \texttt{gamma\_s2c\_burst\_pkts\_std}  & Gamma (Burst) \\
2  & \texttt{alpha\_emb\_38}                & Alpha (Payload) \\
3  & \texttt{gamma\_s2c\_burst\_pkts\_max}  & Gamma (Burst) \\
4  & \texttt{alpha\_emb\_61}                & Alpha (Payload) \\
5  & \texttt{fft\_s2c\_iat\_fft\_mean}      & FFT (Spectral) \\
6  & \texttt{beta\_s2c\_size\_skew}         & Beta (Flow Stats) \\
7  & \texttt{beta\_c2s\_size\_max}          & Beta (Flow Stats) \\
8  & \texttt{gamma\_s2c\_burst\_pkts\_min}  & Gamma (Burst) \\
9  & \texttt{gamma\_s2c\_burst\_pkts\_mean} & Gamma (Burst) \\
10 & \texttt{beta\_c2s\_iat\_max}           & Beta (Flow Stats) \\
\hline
\multicolumn{3}{p{0.9\columnwidth}}{%
\textit{Note:} The top features reveal a hybrid dependency, where Gamma (Burst) features provide the highest information gain, supported by learned Alpha (Payload) embeddings.%
} \\
\end{tabular}
\end{table}
...
To benchmark these results, we compared LE-MVCL against three baselines: two supervised deep learning models (1D-CNN, Deep Packet), and a symmetric contrastive model (SimCLR). Table \ref{tab:baseline_comparison} summarizes the performance gap ($\Delta$) between each baseline and LE-MVCL at the critical 5\% and 30\% thresholds.
...
LE-MVCL consistently outperforms the deep learning baselines in the binary task. At 5\% labels, it exceeds the Supervised 1D-CNN by 6.3\% and Deep Packet by 9.2\%.

Scalability in Fine-Grained Tasks:
In the application identification task, LE-MVCL demonstrates superior long-term performance. At 30\% label density, it achieves the highest F1-score of 91.3\%, surpassing SimCLR (89.5\%). Although the Deep Fusion baseline shows strong initial performance in Application identification at 5\%, LE-MVCL closes the gap at 30\% (91.3\% vs 91.0\%), while consistently outperforming it in the Binary domain (89.8\% vs 85.0\% at 5\% labels), highlighting the robustness of our Direct Fusion strategy.
