...
To assess cross-domain generalization, we utilized the VNAT dataset solely for the Binary Task (Module 1). We employed a Few-Shot Transfer Learning strategy, reusing the pre-trained ISCX encoder and fine-tuning only the classifier head on small subsets of VNAT, to evaluate the robustness of the learned features across different network environments.
...
To validate the three core pillars of our contribution—Label Efficiency, Asymmetric Alignment, and Direct Fusion—we benchmark LE-MVCL against four distinct categories of baselines. This comparative analysis is designed to isolate specific architectural decisions and prove their individual efficacy.

\subsubsection{Group A: The "Necessity of Deep Learning" Baseline}
A critical question in traffic analysis is whether complex Deep Learning is truly required, or if robust feature engineering suffices.
\begin{itemize}
    \item Stats-Only (XGBoost) \cite{chen2016xgboost}:
    A lightweight Gradient Boosting model trained \textit{exclusively} on the concatenated "Greek" statistical features ($\beta, \gamma, \text{FFT}$). The 1D-CNN payload encoder is removed entirely.
    \item \textit{Hypothesis Tested:} This baseline establishes the "performance ceiling" of pure statistical analysis. If LE-MVCL significantly outperforms this model, it proves that the learned payload representations ($\alpha$) capture semantic protocol signatures that are invisible to statistical metrics.
\end{itemize}

\subsubsection{Group B: Supervised Deep Learning (Label Efficiency Check)}
...
\subsubsection{Group C: Symmetric Contrastive Learning (The "Asymmetric" Check)}
...
\subsubsection{Group D: Fusion Architectures (The "Direct Fusion" Check)}
...
\hline
Stats-Only & Stats ($\beta, \gamma, \text{FFT}$) & Supervised (XGB) & Check necessity of Payload \\
1D-CNN & Payload ($\alpha$) & Supervised (DL) & Check Label Efficiency \\
